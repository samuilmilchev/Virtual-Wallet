...
namespace Virtual_Wallet.Repository
{
    public class WalletRepository : IWalletRepository
    {
        ...
        public WalletRepository(ApplicationContext context, IUsersService usersService)
        {
            _context = context;
        ...
        public void CreateSavingWallet(SavingWalletViewModel model)
        {
            User user = _context.Users
                ...
                .Include(u => u.SavingWallets)
                .SingleOrDefault(u => u.Id == model.CurrentUser.Id);

            SavingWallet newWallet = new SavingWallet();

            newWallet.Amount = model.Amount;
            newWallet.StartDate = DateTime.Now;
            newWallet.EndDate = model.EndDate;
            newWallet.Owner = user;
            newWallet.InterestRate = model.InterestRate;

            user.SavingWallets.Add(newWallet);

            _context.SavingWallets.Add(newWallet);
            _context.SaveChanges();
        }
    }
}
